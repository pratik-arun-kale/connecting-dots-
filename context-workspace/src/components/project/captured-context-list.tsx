...
import { MessageSquare, ChevronDown, ChevronUp, StickyNote } from 'lucide-react';
...
function ContextCard({ context }: { context: ApiContext }) {
  ...
  const platform = (context.metadata as Record<string, string> | null)?.platform ?? 'unknown';
  const messages = (context.raw_content as { messages?: ConversationMessage[] })?.messages ?? [];
    ...
    <Card className="border border-border/60 bg-card/45 hover:border-border/80 transition-all">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${
                PLATFORM_COLORS[platform] ?? 'bg-muted/20 text-muted-foreground border-border/40'
              }`}
            >
      ...
      <CardContent className="p-4 pt-2 space-y-2">
        ...
        <div className="pt-1 border-t border-border/20 text-[10px] text-muted-foreground/60 font-mono">
          {messages.length} message{messages.length !== 1 ? 's' : ''} · session {context.session_id.slice(0, 8)}
        </div>
      </CardContent>
    </Card>
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 border border-dashed rounded-xl border-border bg-card/25 text-center">
        ...
        <p className="text-xs text-muted-foreground max-w-xs">
          Open a session in ChatGPT, have a conversation, then click the{' '}
          <span className="font-semibold text-foreground">"Capture Context"</span> button injected
          by the extension.
        </p>
      </div>
    );
